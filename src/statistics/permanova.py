...
def permanova(Y, groups, blocks=None, n_permutations=100, seed=42):
    """
    PERMANOVA on Euclidean distance matrix computed from joint transient representation Y.
    Computes pseudo-F statistic, permutation p-value, and R2_network (explained variance).
    """
    ...
    SSB_obs = SST - SSW_obs
    r_squared = float(SSB_obs / SST) if SST > 0 else 0.0
...
def run_permanova_analysis():
    ...
    res_perm = permanova(Y_joint, groups, n_permutations=99, seed=42)
    print(f"PERMANOVA F-pseudo:   {res_perm['F_pseudo']:.4f}")
    print(f"PERMANOVA p-value:    {res_perm['p_value']:.4f}")
    print(f"PERMANOVA R2_network: {res_perm['r_squared']:.4f}")
